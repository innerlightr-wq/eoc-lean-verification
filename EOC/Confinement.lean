...
/-! ### Coarse-Depth Window Theorem

This section is a corollary-level addition, not part of Propositions 5.7–5.9.
It distinguishes two depths that Proposition 5.2 (`Realizer.lean`, frozen)
conflates informally: `2^(S_N+1)` is needed to pin down the *exact* realizer
(equivalently, exact terminal parity), but only `2^S_N` — one bit less — is
needed to determine the *confinement/exit decision* through step `N`, given
that the prescribed terminal step already exits strictly. Every theorem here
is derived from existing `Realizer.lean` public API (`hdvd_of_aggregate`)
plus local `Confinement.lean` lemmas (`confined_congr`, `s_eq_of_agree`) and
elementary order theory. `Realizer.lean` itself is untouched, per its frozen
status. -/

/-- **Phase 2 (`terminal_valuation_ge`).** The one-sided half of
`exact_val_iff_terminal_odd`, extracted as its own reusable fact. Needs only
the aggregate congruence at `N` (via `hdvd_of_aggregate`) and `N ≥ 1` — no
interior-positivity hypothesis and no oddness hypothesis, since this
direction never invokes `interior_odd` at all. Proof is the identical
derivation already used inside `exact_val_iff_terminal_odd`
(`Realizer.lean`), stopped one step earlier: that theorem needs the full
equality-vs-strictly-larger dichotomy, this one needs only the "≥" half. -/
theorem terminal_valuation_ge (d : ℕ → ℕ) (m0 N : ℕ) (hN : 1 ≤ N)
    (hagg : 2 ^ S d N ∣ (3 ^ N * m0 + q d N)) :
    d (N - 1) ≤ a (iter d m0 (N - 1)) := by
...
/-- **Phase 3 (`drift_mono_of_le`).** Elementary, not Collatz-specific:
increasing a single digit's contribution to drift can only increase drift,
for an arbitrary fixed baseline `Rprev` and fixed shift `shift` (in the
intended use, `shift = alpha`, but nothing here depends on that). -/
theorem drift_mono_of_le {Rprev shift : ℝ} {d d' : ℕ} (hdd' : d ≤ d') :
    Rprev + (d : ℝ) - shift ≤ Rprev + (d' : ℝ) - shift := by
  have hcast : (d : ℝ) ≤ (d' : ℝ) := by exact_mod_cast hdd'
  linarith

/-- **Phase 4 (`coarse_exit_of_ge`).** If a prescribed digit `d` already
forces strict exit past threshold `c`, any true digit `d' ≥ d` forces exit
too. Pure order theory, generic in `Rprev`, `shift`, `c`. -/
theorem coarse_exit_of_ge {Rprev shift c : ℝ} {d d' : ℕ} (hdd' : d ≤ d')
    (hexit : c < Rprev + (d : ℝ) - shift) :
    c < Rprev + (d' : ℝ) - shift := by
  have hmono := drift_mono_of_le (Rprev := Rprev) (shift := shift) hdd'
  linarith

/-- **Phase 5 (`coarse_depth_window`), the Coarse-Depth Window Theorem.**
Suppose `m0` satisfies the aggregate congruence for the length-`N` word `d`
(so, by `exact_val_interior`/`hdvd_of_aggregate`, the dynamics generated by
`iter d m0` matches `d` exactly on the interior and has terminal valuation
`a (iter d m0 (N-1)) ≥ d (N-1)`, by `terminal_valuation_ge`). If the
*prescribed* word `d` is confined through the interior (`Confined c d
(N-1)`) and its own prescribed terminal step exits strictly (`c < R d N`),
then the *true* word — `d` with its terminal digit replaced by the actual
valuation `a (iter d m0 (N-1))`, which may differ from `d (N-1)` exactly
when the two lifts modulo `2^(S_N+1)` disagree — is *also* confined through
the same interior and *also* exits strictly at step `N`. The exact value of
the terminal digit is invisible to this decision; only the "≥" is used,
which is exactly what the extra bit at `2^(S_N+1)` does *not* resolve. -/
theorem coarse_depth_window (d : ℕ → ℕ) (m0 N : ℕ) (c : ℝ) (hN : 1 ≤ N)
    (hagg : 2 ^ S d N ∣ (3 ^ N * m0 + q d N))
    (hconf : Confined c d (N - 1))
    (hexit : c < R d N) :
    Confined c (fun i => if i = N - 1 then a (iter d m0 (N - 1)) else d i) (N - 1)
    ∧ c < R (fun i => if i = N - 1 then a (iter d m0 (N - 1)) else d i) N := by
  set d_true : ℕ → ℕ := fun i => if i = N - 1 then a (iter d m0 (N - 1)) else d i
    with hd_true_def
  have hNsub : N - 1 + 1 = N := by omega
  have hagree : ∀ i < N - 1, d i = d_true i := by
    intro i hi
    have hne : i ≠ N - 1 := by omega
    simp [hd_true_def, hne]
  refine ⟨confined_congr hagree hconf, ?_⟩
  have hge : d (N - 1) ≤ d_true (N - 1) := by
    have heq : d_true (N - 1) = a (iter d m0 (N - 1)) := by simp [hd_true_def]
    rw [heq]
    exact terminal_valuation_ge d m0 N hN hagg
  have hspre : s d (N - 1) = s d_true (N - 1) := s_eq_of_agree hagree (le_refl (N - 1))
  have hsN_d : s d N = s d (N - 1) + d (N - 1) := by
    conv_lhs => rw [← hNsub]
    rw [s_succ]
  have hsN_dtrue : s d_true N = s d_true (N - 1) + d_true (N - 1) := by
    conv_lhs => rw [← hNsub]
    rw [s_succ]
  have hRd : R d N = (s d (N - 1) : ℝ) + (d (N - 1) : ℝ) - (N : ℝ) * alpha := by
    unfold R
    rw [hsN_d]
    push_cast
    ring
  have hRd_true : R d_true N
      = (s d (N - 1) : ℝ) + (d_true (N - 1) : ℝ) - (N : ℝ) * alpha := by
    unfold R
    rw [hsN_dtrue, ← hspre]
    push_cast
    ring
  rw [hRd] at hexit
  rw [hRd_true]
  exact coarse_exit_of_ge (Rprev := (s d (N - 1) : ℝ)) (shift := (N : ℝ) * alpha) hge hexit
