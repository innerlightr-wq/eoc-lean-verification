...
/-- A finite generated probability law remains a genuine generated probability law after
pushforward.  The target atom weight is the finite sum over each fiber. -/
theorem pushforward_genEventProb_isProbabilityLaw_of_finiteSupport {β : Type*}
    (Nwin : ℕ) (w : ℕ → ℝ) (hw_nonneg : ∀ j, 0 ≤ w j)
    (hsupp : Function.support w ⊆ ↑(range Nwin)) (hw_sum : ∑' j, w j = 1)
    (F : ℕ → β) : IsProbabilityLaw (pushforward (genEventProb w) F) := by
  classical
  let w' : β → ℝ := fun v => ∑ j ∈ range Nwin, if F j = v then w j else 0
  have hw'_nonneg : ∀ v, 0 ≤ w' v := by
    intro v
    ...
    · exact hw_nonneg j
    · exact le_refl 0
  have hw'_summable : Summable w' := by
    ...
    apply hvni
    exact Finset.mem_image.mpr ⟨j, hj, hFjv⟩
  have heq : pushforward (genEventProb w) F = genEventProb w' := by
    simpa [w'] using genEventProb_pushforward_fiber_general Nwin w hsupp F
  have huniv : genEventProb w' Set.univ = 1 := by
    rw [← heq]
    unfold pushforward
    rw [Set.preimage_univ]
    exact genEventProb_univ w hw_sum
  have hw'_sum : ∑' v, w' v = 1 := by
    simpa [genEventProb] using huniv
  exact ⟨w', hw'_nonneg, hw'_summable, hw'_sum, heq⟩

/-- The conditional restart law is a genuine discrete probability law whenever its harmonic
normalizer is positive and has the stated finite-sum value. -/
theorem conditionalRestartLaw_isProbabilityLaw (r Dcyl kmin Nwin : ℕ) (W : ℝ)
    (hW : 0 < W) (hr : 0 < r)
    (hWdef : W = ∑ j ∈ range Nwin,
      (1 : ℝ) / ((r : ℝ) + (Dcyl : ℝ) * ((kmin : ℝ) + j)))
    (C t : ℕ) : IsProbabilityLaw (conditionalRestartLaw r Dcyl kmin Nwin W C t) := by
  unfold conditionalRestartLaw
  exact pushforward_genEventProb_isProbabilityLaw_of_finiteSupport Nwin
    (conditionalIndexWeight r Dcyl kmin Nwin W)
    (conditionalIndexWeight_nonneg r Dcyl kmin Nwin W hW hr)
    (conditionalIndexWeight_support r Dcyl kmin Nwin W)
    (conditionalIndexWeight_tsum_eq_one r Dcyl kmin Nwin W hW hWdef) _
...
theorem conditional_future_valuation_mixing_of_constants_at_window
  ...
  have hrpos : 0 < r := by
    obtain ⟨c, hc⟩ := h.1
    omega
  have hWdef' : W = ∑ j ∈ range N,
      (1 : ℝ) / ((r : ℝ) + ((2 ^ (S d t + 1) : ℕ) : ℝ) * ((kmin : ℝ) + j)) := by
    simpa using hWdef
  have hlaw : IsProbabilityLaw
      (conditionalRestartLaw r (2 ^ (S d t + 1)) kmin N W (orbit r t) t) :=
    conditionalRestartLaw_isProbabilityLaw r (2 ^ (S d t + 1)) kmin N W hWpos hrpos hWdef'
      (orbit r t) t
  exact hK n Q (conditionalRestartLaw r (2 ^ (S d t + 1)) kmin N W (orbit r t) t)
    hlaw hn hQrel hoddsupp hres_final
