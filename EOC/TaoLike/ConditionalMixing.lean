...
/-- **`conditional_future_valuation_mixing`.** FORMALLY VERIFIED *conditional on* the
external `TaoMixingHypothesis` interface — composes: (1) the exact deterministic Collatz
cylinder restart (Milestones 1–2), (2) the explicit harmonic residue-TV bound and its
Tao-metric conversion (Milestones 3B, 5), (3) the multiplicative information budget
(Part 7), and (4) Tao's Proposition 1.9 itself (`tao : TaoMixingHypothesis`, Milestones
4/4B) — to conclude: conditioned on a realized prefix `d` of length `t` and a thick
harmonic restart window, *if* the budget `2(1+1/η)·2^(S d t + 2Q) ≤ K.Cres·Y` holds (for
the constants `K` Tao's theorem supplies at this `c0`), then the future valuation vector
(the next `n` digits after the restarted state) is within `K.A·2^(-K.c1·n)` of the concrete
iid `Geom(2)^n` law, in Tao's own `taoL1TV` metric. -/
...
/-- **Event-transfer corollary** (Part 12/14 of the Milestone 5 brief): under the same
hypotheses as `conditional_future_valuation_mixing`, the conclusion transfers to a bound on
...
theorem conditional_future_event_bound
  ...
  obtain ⟨K, kmin, N, W, hWpos, hWdef, hmix⟩ :=
    conditional_future_valuation_mixing tao d t r h ht n hn Y H hYr hYpos η hη hH
      c0 hc0 Q hQrel hQ1 hthick
  refine ⟨K, kmin, N, W, hWpos, hWdef, ?_⟩
  intro hbudget
