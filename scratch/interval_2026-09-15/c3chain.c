...
int main(int argc, char **argv) {
  K = atoi(argv[1]); refined = atoi(argv[2]); gd = argc > 3 ? strtold(argv[3], 0) : 0; int mode = argc > 4 ? atoi(argv[4]) : 0;
  ...
  LD *PM = calloc((size_t)W * (J + 1), sizeof(LD)); LD *cur = calloc(W + 1, sizeof(LD)), *nw = calloc(W + 1, sizeof(LD));
  ...
  for (int sig = 0; sig < K && sig <= bb[J]; sig++) {
    ...
    for (int N = 0; N <= J; N++) { LD a = 0; for (int S = 0; S <= bb[N] && S < W; S++) if (F(N, S) > 0) a += g[(size_t)N * W + S]; rho[N] = a / P; if (mode == 2 && sig == 193 && N % 10 == 0) printf("N=%d log2rho=%.2f mu=%.0Lf l2X=%.1f\n", N, (double)log2l(rho[N]), mu[N], (double)l2X[N]); }
    // prefix-min table PM[u][N] = min(1, min_{N'<=N} v(u,N'))
    for (int u = 0; u < W; u++) { LD run = 1; for (int N = 0; N <= J; N++) {
      LD v = mu[N] * (1 + 2 * powl(3, N) * (1 + N * logl(3)) / powl(2, u)) * rho[N] * powl(2, -2 * gd * (J - N));
    ...
    for (int s = K; s <= sN; s++) {
      ...
      for (int u = 0; u < m; u++) {
        ...
        sum += w * best;
        if (mode == 2 && sig == 193 && s == 219 && u % 8 == 0) printf("u=%d log2a=%.2f log2(w a)=%.2f\n", u, (double)log2l(best), (double)log2l(w*best)); }
