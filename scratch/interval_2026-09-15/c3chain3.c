...
int main(int argc, char **argv) {
  ...
  for (int sig = 0; sig < K && sig <= bb[J]; sig++) {
    ...
    for (int s = K; s <= sN; s++) {
      ...
      if (!heavy) { Hc += H * ctriv; continue; }
      int Nmax = 0; while (Nmax < J && l2X[Nmax + 1] <= m) Nmax++;
      LD sum = 0;
      for (int u = 0; u < m; u++) { LD a = PM[(size_t)u * (J + 1) + Nmax] / (1 - powl(2, -t));
        sum += (refined ? fminl(powl(2, u + 1), powl(2, t)) : powl(2, t)) * a; }
      LD eps = sqrtl((1 + (sig + 1) / 2.0L) * sum * powl(2, t) / Vc);
      Hc += H * fminl(ctriv, 1 + eps);
      if (verb) printf("H=%.3Le sig=%d s=%d t=%d log2V=%.1f log2eps=%.2f log2ctriv=%d\n", H, sig, s, t, (double)log2l(Vc), (double)log2l(eps), s + 1 - K);
    }
  }
  LD C = Hc / Htot, Ct = Htriv / Htot;
  printf("K=%d j0=%d refined=%d gd=%.4f: C=%.4Le (log2 %.2f, /K %.5f) | trivial-only log2 C=%.2f (/K %.5f)\n",
         K, J, refined, (double)gd, C, (double)log2l(C), (double)(log2l(C) / K), (double)log2l(Ct), (double)(log2l(Ct) / K));
  return 0;
}
