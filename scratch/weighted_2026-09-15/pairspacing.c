...
int main(int argc, char **argv) {
  ...
  uint64_t *mm = malloc(8 * Ns); double *zz = malloc(8 * Ns);
  for (long n = 0; n < Ns; n++) {
    ...
    mm[n] = (uint64_t)m;
    { uint64_t T = tmax, mask = (1ULL << T) - 1; uint64_t half = ((uint64_t)(m - 1) >> 1) & mask;
      uint64_t B = ((0 - half) * inv3pow[J]) & mask; long double u = (long double)r / ldexpl(1.0L, sg + 1);
      zz[n] = (double)(((long double)B + u) / ldexpl(1.0L, T)); }
  }
  if (0) for (int t = 4; t <= tmax; t += 2) {
  ...
  qsort(zz, Ns, 8, cmpd);
  printf("pair-spacing E(q)/Haar (sampled pairs within 2^-q, circular), Haar = Ns(Ns-1) 2^{1-q}:\n");
  for (int q = 4; q <= tmax; q += 2) { double w = ldexp(1.0, -q); long double pairs = 0; long jj = 0;
    for (long i = 0; i < Ns; i++) { if (jj < i + 1) jj = i + 1; while (jj < Ns + Ns && ((jj < Ns ? zz[jj] : zz[jj - Ns] + 1) - zz[i]) < w) jj++; pairs += (jj - i - 1); }
    long double haar = (long double)Ns * (Ns - 1) * w;  /* ordered-unordered: pairs counted once each, circular window of total width 2w -> Ns(Ns-1)/2 * 2w */
    printf("  q=%2d ratio=%.4f (pairs %.0Lf, Haar %.0Lf)\n", q, (double)(pairs / haar), pairs, haar); }
  return 0;
}
