...
`python3 haar_bound.py`, Monte Carlo checks `mc_check.py`, `mc_check2.py`), `sparsevisits_2026-09-16/` (exponent-lift structure of
2^{−a} mod 3^D; `python3 lift.py`, `python3 largeA.py`), and `shiftaudit_2026-09-16/` (shift audit and worst-shift
certification behind `EOC/ArithmeticFrontier.lean`; `python3 shifts.py`, `python3 analyze.py`, `python3 certify_worst.py`). For the last one, run `python3 residues.py > residues.txt`
