...
int main(int argc, char **argv) {
  int J = atoi(argv[1]), sg = atoi(argv[2]), t = atoi(argv[3]), U = atoi(argv[4]), m = sg + t + 1, Kb[512];
  for (int j = 0; j < 512; j++) Kb[j] = (int)floorl(j * 1.584962500721156181453738943947816508759814407692481060455L);
  ...
  double P = cnt[0][0];
  Ub inv3, a[512]; for (int k = 0; k < NL; k++) inv3.w[k] = 0xAAAAAAAAAAAAAAAAULL; inv3.w[0] = 0xAAAAAAAAAAAAAAABULL;
  a[0] = inv3; for (int i = 1; i < J + 2; i++) a[i] = mulU(a[i - 1], inv3);   // a[i] = 3^{-(i+1)}
  double complex *f = malloc(16 * D), *g = malloc(16 * D);
  ...
  for (long lam = 1; lam < H; lam++) {
    ...
    for (int i = J - 1; i >= 0; i--) {
      Ub ha = mulU(a[i], L); double complex suf = 0;
      for (int S = D - 1; S >= 0; S--) {  // suf = sum_{S' > S, S' <= b(i+1)} cnt[i+1][S'] f[S']
        g[S] = 0;
        if (S <= Kb[i] && cnt[i][S] > 0) { double x = (double)b64(ha, m - S - 64) * 0x1p-64; g[S] = suf / cnt[i][S] * cexp(-2 * M_PI * I * x); }
        if (S <= Kb[i + 1] && S >= 1) suf += cnt[i + 1][S] * f[S];
      }
      memcpy(f, g, 16 * D); }
    ph[lam] = creal(f[0] * conj(f[0]));
  }
