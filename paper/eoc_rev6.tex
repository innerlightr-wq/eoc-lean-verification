...
while remaining compatible with hypothetical nontrivial cycles and hence strictly weaker than the
Collatz conjecture.
...
Global valuation-word information cannot decide it either: we exhibit an explicitly indexed
uncountable family of zero-confined words with linear negative drift, summable $\sum 2^{R_n}$ and
$O(G)$ occupation shadow, of which all but countably many have no positive-integer realizer.
...
For every fixed $c>0$ there is $C_c$ with $\Occ_c(m_0)\le K\log_2 m_0+C_c$.
\end{conjecture}
...
$m<90$ reaches $1$ by direct inspection (longest: $m=27$, $41$ accelerated steps); hence Collatz
holds. \status{Revision 5}
\end{corollary}
...
verification~\cite{Barina} settles all seeds below $2^{71}$, any nontrivial cycle's minimum member
...
infinite zero-corridor tail. \status{companion work; already formalized in the repository}
\end{proposition}
...
\]
the two agreeing only when $r(D)$ is a power of $2$. \status{proved; formalized, \S\ref{sec:lean}}
...
$L_1=14$ but $\Occ_1=97$. Consequently a complete single-window theory --- even an optimal one ---
does not yield EOC. \status{Revision 5, computational example retained}
\end{remark}
...
If $R_n\le0$ then $m_n>m_0$. Hence an orbit with $R_n\le0$ for all $n\ge1$ has its seed as a
\emph{strict minimum of its entire future}. Contrapositively, $m_n\le m_0$ already forces
$R_n>0$.
\end{proposition}
...
\begin{proof}
($\Leftarrow$, contrapositive.) If a divergent orbit exists, Proposition~\ref{prop:lastmax}
produces an actual positive odd integer $m^*$ with $R'_k<0$ for every $k\ge1$, so
$\tau_0(m^*)=\infty$ and $(\mathrm{DE})$ fails.

($\Rightarrow$, contrapositive.) Suppose $(\mathrm{DE})$ fails at $m$, so $2^{S_n}\le3^n$ for all
...
$(\mathrm{DE})$ is compatible with hypothetical nontrivial positive cycles: by
Lemma~\ref{lem:cycledrift} every point of a cycle, and every transient point entering one, has
finite $\tau_0$. Hence $(\mathrm{DE})$ is \emph{strictly weaker} than the Collatz conjecture, which
also excludes cycles.
\end{corollary}
...
i.e. it implies the absence of divergent orbits --- and, with that, an $O(\log m)$ bound on
accelerated total stopping time. Conversely a bound $L\le K'\log_2 m$ returns the hypothesis for
every $A\ge0$. \status{proved}
\end{corollary}

So the corridor-uniform hypothesis is \emph{noncircular} with respect to Open Problem~C --- it is
not a repackaging of the realizer floor --- but it is substantially \emph{stronger} than the
Collatz conjecture. It should not be presented as a plausibly easier closing hypothesis. The
...
$R=0$ requires --- is $+1.83$, never negative. \status{the inequality is proved; the margin is a
computational diagnostic}
\end{remark}
...
\subsection{Global word shadows do not characterize positive realization}\label{sec:cardinality}
...
\emph{no} positive-integer realizer. \status{proved; formalized, \S\ref{sec:lean}}
\end{theorem}
...
Every global condition expressible from the valuation word alone --- zero confinement, linear
negative drift, summability of $\sum2^{R_n}$, Curry-scale occupation shadows, aperiodicity,
subword complexity --- is satisfied by all members of this family, while all but countably many are
unrealizable. Hence \emph{no collection of word-level global properties can characterize
positive-integer realization}. This is a statement about characterization, not about quantitative
bounds: it does not preclude word-level information yielding realizer lower bounds. \status{scope
statement}
\end{remark}
...
4 & Total occupation $\Occ_c(m)=O(\log m)$ & Full EOC (Conjecture~\ref{conj:exist}); strictly
    stronger than Level~3 (Rem.~\ref{rem:LvsO}). \\
...
Remark~\ref{sec:nounconditional}. No converse holds in general. Level~1 is strictly weaker than
Collatz (Cor.~\ref{cor:cycles}), and Level~4 is strictly stronger than Level~3.
...
Exclude divergence & Word shadows insufficient (Thm.~\ref{thm:cardinality}) \\
...
realization of such words remains open and, by Theorem~\ref{thm:cardinality}, cannot be decided by
word-level information at all. The missing restriction is not local symbolic dynamics.
...
\midrule
1 & Fixed-modulus drift packing & Terminal at $8/9$ (Rem.~\ref{rem:terminal}); surpassed by a
different mechanism & \textsf{T} \\
...
10 & External $p$-adic arithmetic & Wrong complexity scale (\S\ref{sec:external}) & \textsf{I} \\
11 & Global word shadows & Cardinality barrier (Thm.~\ref{thm:cardinality}) & \textsf{T} \\
...
$m^*\le2^{71}$ by \cite{Barina}; open beyond.
\end{openproblem}
...
Theorem~\ref{thm:DEequiv} this is equivalent to excluding divergent orbits and is strictly weaker
than Collatz. This is the previously posed zero-corridor realizability frontier, reclassified as
...
Control the number of separated confined episodes. Single-window theory does not capture this
(Remark~\ref{rem:LvsO}); full EOC requires both episode length and episode count.
\end{openproblem}
...
existence; zero-corridor tail; deficit recurrence. \\
...
Lift digits are binary blocks; discrete gap and its bracket $g=\lceil\Edep\rceil$. \\
\texttt{TransportRegeneration}, \texttt{SuperBudgetCancellation} & \S\ref{sec:collapse} &
Transport bookkeeping; forced/excess split; disjoint-run bound. \\
...
Statements in this paper that are \emph{not} formalized include Theorem~\ref{thm:GTC} and
Proposition~\ref{prop:recip} (external input), the proved-sector floors of \S\ref{sec:residue}
(companion note), and every conjecture.
...
\item \emph{Drift-exit diagnostics.} $\tau_0=\sigma$ held for every odd seed tested away from cycle
  minima, consistent with Remark~\ref{rem:howweak}.
\end{itemize}
...
\cite{SimonsdeWeger} and Hercher \cite{Hercher}; verification is Ba\v{r}ina \cite{Barina};
...
EOC is best viewed as a programme with an exact residue--time duality and a strictly stronger
global occupation layer. The residue axis is solved for finite realizability and for several
...
divergent orbits, and strictly weaker than Collatz --- and quantitative lifetime bounds that invert
...
the natural divisibility object are complementary factors of one identity; global valuation-word
shadows cannot characterize positive realization, by an explicit uncountable family; launch
...
with Logarithmic-Floor Exclusion beyond the Harmonic Barrier}, Zenodo, 2026.
...
\bibitem{Barina} D. Ba\v{r}ina, \emph{Improved verification limit for the convergence of the
Collatz conjecture}, The Journal of Supercomputing \textbf{77} (2021), 2681--2688; and
\textbf{81} (2025), 810.
