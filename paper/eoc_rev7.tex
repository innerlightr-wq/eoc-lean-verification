...
Let $m_0$ be a positive odd integer whose accelerated orbit is injective. Then for every $N\ge2$,
\[
...
\begin{remark}
The constant is not optimal. Numerically $A_M-\tfrac13\ln M$ increases to $0.9728\ldots$, with
maximum $1$ at $M=1$, so $A_M\le\tfrac13\ln M+1$ and the additive constant can be taken below
$0.97$; Step~5's integral comparison loses $\tfrac13\ln3$. The exponent $\tfrac19$ --- the only part
Theorem~\ref{thm:89} uses --- is unaffected. Over odd $m_0<60001$ the largest observed value of
$E_N-(\tfrac19\log_2 N+1.138)$ is $-1.004$, so the stated bound is comfortable but not tight.
\status{the bound is proved; the remark on sharpness is a computational observation}
\end{remark}
