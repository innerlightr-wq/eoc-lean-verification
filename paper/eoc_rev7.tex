...
single-window confinement lifetime $L_c(m)$, so every nondecreasing pointwise lifetime bound
transfers to a
...
while the drift-exit argument supplies no cycle exclusion: no implication from it to cycle-freeness
is established here, and none is claimed in either direction
...
corridor width is computed rather than assumed, essentially an $O(\log m)$-scale pointwise
lifetime conjecture --- it bounds the \emph{injective} initial segment, so on its own it yields
entry into \emph{a} cycle, not arrival at $1$; and deep negative drift is self-financing, since the ordinary height it creates is
...
periodic and --- \emph{once effective constants are supplied}, which
Conjecture~\ref{conj:exist} does not itself provide --- reduces Collatz to a finite cycle search
...
divergent orbits --- sits beside both. The paper is therefore organized around four axes.
...
\big\downarrow\ \text{\scriptsize$\Occ_c\ge L_c$ pointwise (Rem.~\ref{rem:LvsO})}\\[2pt]
...
\]
\end{minipage}
\caption{The programme map. Vertical double arrows are exact equivalences; single arrows are
implications. The top arrow records the \emph{pointwise} inequality $\Occ_c\ge L_c$; it does
\emph{not} assert that the two universal asymptotic bounds are inequivalent, which is open
(Rem.~\ref{rem:strictness}). The lower chain is the qualitative divergence reduction. Every
implication is referenced in \S\ref{sec:map}.}
\label{fig:map}
\end{figure}
...
\item \textbf{Hierarchy strictness audited} (Rem.~\ref{rem:strictness}). ``No converse holds in
  ...
  For the corridor-uniform form the implication in fact reverses, \emph{given cycle exclusion}.
...
\begin{enumerate}[leftmargin=1.6em]
\item A general transfer principle (\S\ref{sec:transfer}) turning any nondecreasing pointwise lifetime bound into
...
For fixed $c\ge0$, $\Occ_c(m_0)=\#\{n\ge0:R_n\le c\}\in\mathbb{Z}_{\ge1}\cup\{\infty\}$; the four
conjectures below fix $c>0$, and statements using $c=0$ say so. This is a
...
explicit would supply Conjecture~\ref{conj:eff} and, with it, Collatz \emph{provided} the
accompanying cycle search below $m^*$ is completed --- at the constants of
Corollary~\ref{cor:explicit} it is, since $m^*=90$; at other constants $m^*$ may exceed the
verified range (Open Problem~\ref{op:B}). This is Open Problem~\ref{op:A}.
...
$E_N\le\frac19\log_2 N+O(1)$, so $2^{E_N}\ll N^{1/9}$.
\status{Revision 5, Lem.~4.3 --- where it carries a proof \emph{sketch} only. Theorem~\ref{thm:89}
below rests on it, so a complete elementary proof with an explicit constant is given in
Appendix~\ref{app:carrybudget}}
\end{lemma}
...
infinitely many $k$. \status{Revision 5, Thm.~4.5, verbatim; fully proved there, but via
Lemma~\ref{lem:carrybudget}, which Revision 5 only sketches}
\end{theorem}
...
For a fixed prefix $Y$ the same mechanism applies to $YX^n$ with transformed anchor $\xi_{X,Y}$:
if $\xi_{X,Y}\notin\Zpos\cup\{0\}$ then
$r(YX^n)\ge2^{S_Y+nS_X-B_{X,Y}}$ \emph{for every $n$ with $S_Y+nS_X\ge P_{X,Y}+B_{X,Y}$}, and
$\log_2 r(YX^n)/(S_Y+nS_X)\to1$.
\status{Revision 5; proved unconditionally in \cite{DJperiodic}, Thm.~9.1. The exceptional case is
\emph{not} literally the same as in Thm.~\ref{thm:periodic}: there $\xi_X\ne0$ holds for every
block, whereas $\xi_{X,Y}\ne0$ is proved in \cite{DJperiodic} only when $b_X>1$ (Cor.~10.2), the
boundary case $b_X=1$ being checked directly when it arises}
\end{theorem}

\subsection{The finite-defect sector: an unproved claim inherited from Revision 5}
\label{sec:bulkdefect}

Nothing in this paper depends on the statement below. It is retained only because Revision~5
recorded it and a reader of that revision will look for it; it is \textbf{not} a result of this
programme, and it is placed outside \S\ref{sec:residue}'s proved sectors deliberately.

\begin{proposition}[Bulk-defect anchor stability --- \textbf{stated, not proved}]\label{prop:bulkdefect}
...
exceptional case, $r(D)\ge2^{aS_X-B_X}$ \emph{for $a$ sufficiently large}.
\status{Revision 5, Prop.~5.10. \textbf{No proof is on file} --- see Rem.~\ref{rem:bulkdefectopen}}
\end{proposition}

\begin{remark}[The finite-defect sector is open, and this proposition is not a proved
sector]\label{rem:bulkdefectopen}
Revision~6 and the first draft of Revision~7 listed Proposition~\ref{prop:bulkdefect} under
``proved sectors''. An audit of the sources withdraws that classification.

\begin{enumerate}[label=(\roman*),leftmargin=2em]
\item The companion note \cite{DJperiodic}, which this subsection cites for the periodic sector,
  contains \emph{no} statement about defect words. Its epistemic ledger lists ``Finite-defect
  words --- \textbf{Open, major}'', and its limitations section says of exactly this class: ``This
  note does not attempt to resolve this question.'' It further warns that it ``does not establish
  $r(D)\ge2^{S(D)-O(1)}$ for arbitrary confined words, and no such claim should be inferred from
  the results proved here.''
\item Revision~5 states Proposition~5.10 with a one-paragraph mechanism and \emph{no proof}, and
  carries the qualifier ``for $a$ sufficiently large'', which Revision~7 had dropped and which is
  genuinely needed (the analogue in Thm.~\ref{thm:periodic} holds only once $nS_X\ge P_X+B_X$).
\item The statement is not contradicted by (i): its content concerns the coarse anchor at depth
  $aS_X$, fixed by the $X^a$ prefix alone, and its floor is exponential in $a$ only, not in
  $|D|=aS_X+|E|+bS_X$. It is weaker than what the note calls open. But weaker-and-unproved is not
  proved.
\end{enumerate}

We therefore retain the statement, as a recorded Revision~5 claim, and remove it from the proved
sectors. Theorems~\ref{thm:periodic} and~\ref{thm:evperiodic} are unaffected: both are proved
unconditionally in \cite{DJperiodic} (Thms.~6.2 and~9.1). \status{Revision 7, correction of status}
\end{remark}
...
Periodic and eventually periodic words have $\rho_N\to0$, at rate $1/(\text{number of
repetitions})$: a fixed height is amortized over unboundedly many periods. (The same amortization
is what Proposition~\ref{prop:bulkdefect} would give for one-defect words, but that proposition is
unproved --- Rem.~\ref{rem:bulkdefectopen}.) Arbitrary confined words
...
\emph{given cycle exclusion} (Rem.~\ref{rem:Uallscope}(i)).
...
\end{enumerate}
\status{Revision 5, Prop.~5.7, verbatim. Revision 5 labels its argument a proof \emph{sketch}; it is
complete in substance in both directions, and both directions are re-derived in the Revision 7 audit}
\end{proposition}
...
otherwise: \emph{any} unconditional pointwise lifetime bound by a nondecreasing $F$, however weak,
yields a corresponding unconditional frontier bound. A bound as weak as $L_c(m)\le Km$ would already give a linear
...
Collatz additionally requires excluding nontrivial cycles, and whether $(\mathrm{DE})$ already does
so is precisely the open cycle question --- see Remark~\ref{rem:strictlyweaker}, which claims
nothing either way.
\end{corollary}
...
Call $D$ of length $N$ \emph{$\varepsilon$-dangerous} if it is $c$-confined, has $r(D)\ge3$, and
satisfies $\log_2 r(D)<\varepsilon N$ (with $c\ge1$ and $\varepsilon\in(0,1)$). For such a word the
pinning index $t_1$ and the launch data satisfy
\[
...
in the launch-size bound is $\al\varepsilon$, not $\varepsilon$.

\begin{remark}[Hypotheses the launch-size bound carries]\label{rem:launchhyp}
The third bound --- the launch-size bound --- holds in \cite{DJlaunch} (Prop.~6.3(d)) only
\emph{if the orbit segment $r(D)=m_0,\dots,m_{t_1}$ is injective}. That hypothesis is not free. The
note closes the trivial-cycle branch unconditionally and sharply for $N>N_c$, but records, in a
retraction worth quoting, that the other branch is open: an earlier draft's claim that cycle
exclusions render a nontrivial-cycle tail incompatible with danger ``was incorrect, with the
asymptotics running in the wrong direction, and is retracted \dots\ the cycle branch is closed
unconditionally \dots\ for the trivial cycle, and open for hypothetical nontrivial cycles at large
$N$.'' The note then carries the hypothesis explicitly rather than, in its own words, assuming
nonexistence silently.

Revision~6 and the first draft of Revision~7 stated the launch-size bound flatly. It is restated
here with its hypothesis. The renormalization-stability sentence below inherits the same condition,
together with \cite{DJlaunch}'s $N>N_c$, its period cap $q_0$, and the threshold
$\varepsilon<(2-\al)/(2+\al)\approx0.1158$ below which its terminal-repetition bound is useful.
\status{Revision 7, restored hypotheses}
\end{remark} The residual irregular class is
...
$\mathcal{C}$.
\begin{enumerate}[label=(\roman*),leftmargin=2em]
\item If $\max\mathcal{C}<m_02^{-c}$ then $\Occ_c(m_0)\le T$.
\item In particular, if the orbit reaches $1$ at time $n^*\ge1$ and $m_0>2^{c}$, then
  $\Occ_c(m_0)\le n^*$.
\end{enumerate}

\emph{Proof.} By Proposition~\ref{prop:corridorvalue} every corridor time $n$ satisfies
$m_n\ge m_02^{-c}$; under (i) no $n\ge T$ does, so all corridor times precede $T$. For (ii),
$\mathcal{C}=\{1\}$ and $1<m_02^{-c}$ exactly when $m_0>2^{c}$. \qed

\begin{remark*}[The value criterion is vacuous in the remaining case]
Revision~7 first stated this as $\Occ_c(m_0)\le T+\#\{n\ge T: m_n\ge m_02^{-c}\}$. That inequality
is true but \emph{useless} outside case~(i): if any cycle value meets the threshold, the cycle
repeats it forever, so the counting term is \textbf{infinite}, while the actual occupation is
finite. Occupation is not governed by the value criterion alone --- corridor membership is a
condition on $R_n$, and by Lemma~\ref{lem:cycledrift} $R$ gains a fixed positive amount per period,
so the corridor is left permanently after finitely many periods. That finite bound depends on the
cycle --- its period, its per-period drift gain, and the drift at entry --- and we have no bound on
it uniform over hypothetical nontrivial cycles. The display has been removed rather than retained
with a caveat. \status{Revision 7, correction}
\end{remark*}
...
$O(\log m_0)$. If the only positive cycle is $\{1\}$ --- that is, \emph{given cycle exclusion} ---
then every orbit reaches $1$ by time $T$, and Proposition~\ref{prop:occstop}(ii) gives
$\Occ_c(m_0)\le T=O(\log m_0)$ for every $m_0>2^{c}$: the corridor-uniform hypothesis implies
Existence EOC at every $c$. Under the same additional assumption it also implies the Collatz
conjecture, so it relocates the difficulty rather than reducing it.
...
\subsection{Single-window lifetime is not the same functional as occupation}

Remark~\ref{rem:LvsO}. This concerns the two \emph{functionals}, witnessed by one seed; it is
\emph{not} the claim that the universal bounds are inequivalent, which is open
(Rem.~\ref{rem:strictness}). \status{Revision 5, computational example}
...
1 & Universal drift exit $(\mathrm{DE})$ & Equivalent, given the external Thm.~\ref{thm:GTC}, to
    excluding divergent orbits
    (Thm.~\ref{thm:DEequiv}); no implication to cycle-freeness established
    (Rem.~\ref{rem:strictlyweaker}); \emph{open}. \\
...
3 & Exponential single-window floor & Open Problem~\ref{op:C}; equivalent to
    $L_c(m)=O(\log m)$ for odd $m\ge3$, words taken with $r(D)\ge3$
    (Prop.~\ref{prop:window}). \\
...
Fixed-anchor sectors (Thms.~\ref{thm:periodic}--\ref{thm:evperiodic}) & Finite-defect
(\S\ref{sec:bulkdefect}) and arbitrary irregular floors & Coordinate collapse
(\S\ref{sec:collapse}) \\
...
structural argument about primes $p>3$ (Rem.~\ref{rem:terminal}), not a theorem about all moduli;
surpassed by a different
...
12 & Corridor-uniform lifetime hypothesis & Bounds the injective initial segment on an
$O(\log m)$ scale (Cor.~\ref{cor:Uall}); reaching $1$ needs cycle exclusion
(Rem.~\ref{rem:cyclegap}) & \textsf{T} \\
...
admissible constants in Conjecture~\ref{conj:exist}. With the cycle search below the resulting
$m^*$ also completed, this closes the Collatz implication --- Corollary~\ref{cor:explicit} does
complete it at $m^*=90$; in general that search is Open Problem~\ref{op:B}.
\end{openproblem}
...
Prove \emph{any} unconditional pointwise upper bound $L_c(m)\le F(m)$ with $F$ nondecreasing. By
...
Let $e_0=e_0(c)$ be the first exit time of the oscillating word below. For every $c\ge0$, every
$B$, and every prescribed bound $M$, there are $N$ and a positive odd $m_0>M$ whose genuine
accelerated orbit follows that word for $N$ steps and satisfies
\[
\#\{\text{corridor re-entries before }N\}\ \ge\ B,
\qquad
\#\{n<N:\ R_n\le c\}\ \le\ e_0+2B .
\]
Both conclusions concern the realized prefix $[0,N)$ only.
...
within three steps, so exits recur at gaps of at most $3$ and $N\le e_0+3B+1$ suffices for $B$
re-entries; the prefix occupation is then $N$ minus the number of exits, which is at most
$e_0+2B$. Every finite prefix is realized by a positive odd integer via
Proposition~\ref{prop:congruence}, and since the realizers of a prefix form a full residue class,
one may take $m_0>M$ without changing the word or either count. \qed
\status{Revision 7; formalized, \S\ref{sec:lean}}
\end{proposition}
...
\emph{One particular coarse accounting is lossy, by a stated factor.} Fix $K$ and suppose the
corridor-uniform single-window bound charges each episode $\ell_i\le1+K(\log_2m_{a_i}+c_i)$. Define
the \emph{surrogate total} for the realized prefix as the sum of those charges,
\[
Q\ :=\ \sum_{i=1}^{P}\Bigl(1+K\bigl(\log_2 m_{a_i}+c_i\bigr)\Bigr),
\qquad P=\#\{\text{episodes in }[0,N)\},
\]
so that $\Occ_c^{\mathrm{prefix}}\le Q$ is precisely what this route would use. Since
$R_{a_i}\le c$ and $E_{a_i}\ge0$ give $\log_2m_{a_i}+c_i\ge\log_2m_0-c$, every charge is at least
$\log_2m_0-c$, whence $Q\ge P(\log_2m_0-c)$.

Now write $e_0$ for the first exit time of the oscillating word (a constant depending on $c$ alone).
Proposition~\ref{prop:episodes} supplies, for each $B$ and each prescribed bound, an odd realizing
seed \emph{exceeding that bound} whose prefix has $P\ge B$ episodes and prefix occupation
$\Occ_c^{\mathrm{prefix}}\le e_0+2B$. Since $(2+e_0)(B+1)\ge e_0+2B$,
\[
\frac{Q}{\Occ_c^{\mathrm{prefix}}}\ \ge\ \frac{B(\log_2m_0-c)}{e_0+2B}\ \ge\
\frac{\log_2m_0-c}{2+e_0},
\]
in which $B$ cancels. Along a sequence of such seeds with $m_0\to\infty$ the ratio is therefore
unbounded: \emph{this} accounting overestimates the prefix occupation it bounds by a factor
$\Omega(\log m_0)$, and no choice of $K$ repairs it.

Two things this does \emph{not} say. It is not a statement about per-episode methods in general ---
only about the single-window charge just displayed; a different per-episode estimate is not
excluded. And it is not an impossibility theorem about EOC.

\emph{A distinction the argument depends on.} The seeds above are obtained by moving \emph{within
the residue class} of a fixed prefix (Prop.~\ref{prop:congruence}), which makes $m_0$ as large as
desired while leaving the word, and hence $P$ and $\Occ_c^{\mathrm{prefix}}$, unchanged. That is not
the same as showing the \emph{least} realizer grows, and nothing here asserts that it does.
...
orbit? By \S\ref{sec:symbolic} no finite symbolic restriction settles this, and neither do the
\emph{known divergent-orbit} word shadows (Thm.~\ref{thm:cardinality}); word-level criteria in
general are \emph{not} excluded (Rem.~\ref{rem:zcrecounter}). Demoted here from a top-level EOC problem to an auxiliary one.
\end{openproblem}
...
existence (from $R_n\to-\infty$ alone, so not itself needing \texttt{ReciprocalSummable});
zero-corridor tail and deficit results, these under
...
injectivity; the \emph{abstract} countability argument (not instantiated at $w_b$). \\
...
(\texttt{corridor\_value\_lower}); occupation bounded by the arrival time $n^*$ \emph{under}
$1\le n^*$, arrival at $1$ from $n^*$ onwards, and $2^{c}\le m_0$; the oscillating word and unbounded realizable episode counts. \\
...
closure is essentially an $O(\log m)$-scale pointwise lifetime conjecture, bounding the injective
initial segment rather than the time to reach $1$; and deep negative drift is
...
\section{A complete proof of the carry budget, with an explicit constant}\label{app:carrybudget}

Lemma~\ref{lem:carrybudget} carries only a proof sketch in Revision~5, and
Theorem~\ref{thm:89} rests on it, which in turn feeds Corollary~\ref{cor:injocc} and
Theorem~\ref{thm:reduction}(i). Since that is a load-bearing chain, we give the argument in full.
Nothing here is new mathematics; it is the sketch completed, with the constant made explicit so a
reader can check it.

\begin{proposition}\label{prop:carryexplicit}
Let $m_0$ be a positive odd integer whose accelerated orbit is injective. Then for every
$N\ge2$,
\[
E_N\ \le\ \tfrac19\log_2(3N-5)+\frac{2}{3\ln2}\ \le\ \tfrac19\log_2 N+1.138 .
\]
The restriction $N\ge2$ is needed for the first expression to be defined: $3N-5$ is $1$ at $N=2$
and negative at $N=1$. The two remaining cases are immediate and require no hypothesis:
$E_0=0$ (empty sum), and $E_1=\log_2\bigl(1+\tfrac1{3m_0}\bigr)\le\log_2\tfrac43<0.416$.
\end{proposition}

\begin{proof}
\emph{Step 1: the iterates avoid $3$.} For $k\ge1$, $2^{d_{k-1}}m_k=3m_{k-1}+1\equiv1\pmod 3$, and
$2$ is a unit mod $3$, so $m_k\not\equiv0\pmod3$. Each $m_k$ is odd, so $\gcd(m_k,6)=1$ for every
$k\ge1$. (The seed $m_0$ is excluded: it may be divisible by $3$.)

\emph{Step 2: linearise the logarithm.} From $\ln(1+x)\le x$ for $x\ge0$,
\[
E_N=\sum_{k<N}\log_2\Bigl(1+\frac{1}{3m_k}\Bigr)\ \le\ \frac{1}{3\ln2}\sum_{k<N}\frac{1}{m_k}.
\]

\emph{Step 3: injectivity turns the sum into a reciprocal sum over distinct admissible values.}
Let $a_1<a_2<\cdots$ enumerate the positive integers coprime to $6$, and put
$A_M=\sum_{j\le M}1/a_j$. By Step~1 and injectivity, $m_1,\dots,m_{N-1}$ are $N-1$ \emph{distinct}
positive integers coprime to $6$, so
$\sum_{1\le k<N}1/m_k\le A_{N-1}$, whence $\sum_{k<N}1/m_k\le 1+A_{N-1}$ using $m_0\ge1$.

\emph{Step 4: $a_j\ge3j-2$.} Equivalently $\#\{n\le3j-3:\gcd(n,6)=1\}\le j-1$. Each block of six
consecutive integers contains exactly two integers coprime to $6$ (residues $1$ and $5$). Write
$3j-3=6q+s$ with $0\le s<6$. Since $3j\equiv0$ or $3\pmod 6$, we have $s\in\{0,3\}$ --- the cases
$s\in\{1,2,4,5\}$ do not occur. If $s=0$ the count is $2q=(3j-3)/3=j-1$; if $s=3$ the count is
$2q+1=(3j-3)/3=j-1$. In both cases the bound holds.

\emph{Step 5: sum.} Since $t\mapsto1/(3t-2)$ is decreasing on $t>2/3$,
\[
A_M\ =\ 1+\sum_{j=2}^{M}\frac1{a_j}\ \le\ 1+\sum_{j=2}^{M}\frac1{3j-2}
\ \le\ 1+\int_1^M\frac{dt}{3t-2}\ =\ 1+\tfrac13\ln(3M-2).
\]

\emph{Step 6: combine.} With $M=N-1$, Steps 2, 3 and 5 give
\[
E_N\ \le\ \frac{1}{3\ln2}\Bigl(2+\tfrac13\ln(3N-5)\Bigr)
\ =\ \tfrac19\log_2(3N-5)+\frac{2}{3\ln2},
\]
and $\tfrac19\log_2(3N-5)\le\tfrac19\log_2 N+\tfrac19\log_23$ with
$\tfrac19\log_23+\tfrac{2}{3\ln2}=0.1761\ldots+0.9618\ldots<1.138$.
\end{proof}

\begin{remark}[Sharpness, as a computation only]
The constant is not optimal, and Step~5's integral comparison is where most of the slack is: it
loses $\tfrac13\ln3=0.366\ldots$. The exponent $\tfrac19$ --- the only part
Theorem~\ref{thm:89} uses --- is unaffected by any of this.

Computed values of $A_M-\tfrac13\ln M$: it equals $1$ at $M=1$, drops to $0.9690$ at $M=2$, and
from there \emph{increases}, approaching $0.9728\ldots$; so the sequence is not monotone from
$M=1$, and its maximum over the computed range is the first term. (An earlier draft said it
``increases to $0.9728$, with maximum $1$ at $M=1$'', which is self-contradictory as stated.) For
$M\le3\cdot10^{5}$ we find $A_M\le\tfrac13\ln M+1$ without exception, which would give an additive
constant below $0.97$ --- but this is a finite check, not a proof, and no such inequality is used
above. Over odd $m_0<60001$ the largest observed value of $E_N-(\tfrac19\log_2 N+1.138)$ is
$-1.004$, so the proved bound is comfortable but not tight.
\status{Proposition~\ref{prop:carryexplicit} is proved; every numerical statement in this remark is
a finite computation over the stated range and is used nowhere}
\end{remark}
...
\begin{theorem}[Band rigidity, Revision~5 Thm.~A.1]\label{thm:band}
Let $\theta=\al-1$. If a word's drift path satisfies $\beta\le R_j<\beta+1$ for all $j\le\ell$ and
some $\beta\in\mathbb{R}$, then every letter lies in $\{1,2\}$ and, writing $b_i=d_i-1$, the word is
a length-$\ell$ factor of the Sturmian system of slope $\theta$; conversely every such factor
arises, and there are exactly $\ell+1$ admissible words for each $\ell$.
\status{Revision 5, Thm.~A.1, restated verbatim. Revision 5 gives a proof \emph{sketch}, with the
factor characterisation and the count $\ell+1$ cited to Morse--Hedlund (see \cite{Lothaire})}
\end{theorem}

\begin{remark}[Why the substitution $b_i=d_i-1$ and the slope $\theta$ matter]\label{rem:bandslope}
The reduction is exact: with $d_i\in\{1,2\}$ and $b_i=d_i-1\in\{0,1\}$,
\[
R_j=\textstyle\sum_{i<j}d_i-j\al=\sum_{i<j}(b_i+1)-j\al=\sum_{i<j}b_i-j(\al-1)=\sum_{i<j}b_i-j\theta,
\]
so the band condition is the Beatty condition \emph{at slope $\theta$}, on the binary word $b$.
Revision~7 initially paraphrased this as ``the placement follows the Beatty gap sequence of $\al$'',
dropping the substitution and naming the wrong slope, and dropped both the converse and the exact
count while labelling the result ``retained''. It was a paraphrase, not a retention; the statement
above is Revision~5's. \status{Revision 7, correction of attribution}
\end{remark}

The relevance to this revision is limited but worth recording: band rigidity says that a
\emph{narrow} corridor forces exactly Sturmian structure, which is exactly the regime where
