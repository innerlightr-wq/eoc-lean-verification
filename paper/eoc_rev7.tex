...
Collatz conjecture. (Whether it is \emph{strictly} weaker than Collatz is itself open ---
...
$(\mathrm{DE})$ does not exclude hypothetical nontrivial positive cycles: by
Lemma~\ref{lem:cycledrift} every point of a cycle, and every transient point entering one, has
finite $\tau_0$. Hence a proof of $(\mathrm{DE})$ alone would not yield the Collatz conjecture,
which also excludes cycles.
\end{corollary}
...
What Corollary~\ref{cor:cycles} does establish is a statement about \emph{entailment}, not about
strictness: $(\mathrm{DE})$ does not entail the absence of cycles, so no proof of $(\mathrm{DE})$
can by itself deliver Collatz. That is the useful content, and it is what the programme actually
relies on. This is the same error of form corrected in Remark~\ref{rem:strictness}: a
non-implication of \emph{mechanism} does not give strictness of \emph{content}.
...
What survives: Level~4 implies Level~3 (immediately, since $L_c\le\Occ_c$); Level~1 does not
entail the absence of cycles, so no proof of it alone yields Collatz (Cor.~\ref{cor:cycles}) ---
though, as Remark~\ref{rem:strictlyweaker} records, \emph{strict} weakness there is open too; and
for a \emph{fixed} corridor the converse Level~$3\Rightarrow$~Level~4 is open. \status{Revision 7, correction of scope}
\end{remark}
...
Theorem~\ref{thm:DEequiv} this is equivalent to excluding divergent orbits; it does not exclude
cycles, though whether it is \emph{strictly} weaker than Collatz is open
(Rem.~\ref{rem:strictlyweaker}). This is the previously posed zero-corridor realizability frontier, reclassified as
...
divergent orbits, and not excluding cycles --- and quantitative lifetime bounds that invert
