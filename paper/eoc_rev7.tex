...
Nor do the natural global valuation-word shadows decide it: we exhibit an explicitly indexed
...
\item \textbf{Separated returns, made precise} (Open Problem~\ref{op:F}). The entry lemma pins
  ...
  throughout.
\item \textbf{The corridor-uniform hypothesis, clarified} (Rem.~\ref{rem:Uallscope}). It implies
  Existence EOC outright, via Proposition~\ref{prop:occstop}, not merely a lifetime bound; the
  analytic absorption inside Corollary~\ref{cor:Uall} is identified as the unformalized layer; and
  it must not be conflated with a bound at one fixed corridor.
...
\begin{proposition}[Occupation ends at arrival]\label{prop:occstop}
Let $m_0$ be a positive odd seed whose orbit reaches $1$ at time $n^*$. Then
\[
\Occ_c(m_0)\ \le\ n^*+3\cdot2^{c}+1 ,
\]
and if moreover $m_0\ge2^{c}$ the tail contributes nothing, so $\Occ_c(m_0)\le n^*$.

\emph{Proof.} Once $m_n=1$ the map fixes it and every subsequent valuation digit is $2$, so
$S_{n^*+k}=S_{n^*}+2k$, while $2^{S_{n^*}}=3^{n^*}m_0+C_{n^*}\ge3^{n^*}$ by
Lemma~\ref{lem:aggregate}. Corridor membership at $n^*+k$ therefore forces $4^k\le2^{c}3^k$, which
bounds $k$. If $m_0\ge2^{c}$ the same identity gives $2^{S_n}=3^nm_0+C_n>2^{c}3^n$ whenever
$m_n=1$ and $n\ge1$, so no such time lies in the corridor at all. \qed
...
\emph{(i) The hypothesis implies EOC, not merely a lifetime bound.} Combining
Corollary~\ref{cor:Uall} with Proposition~\ref{prop:occstop}: the corridor-uniform hypothesis gives
an $O(\log m)$ total-stopping-time bound, and occupation is confined to the times before arrival at
$1$, so $\Occ_c(m)=O(\log m)$. The hypothesis therefore already implies Existence EOC at every
$c$, with no auxiliary recurrence assumption. This relocates the difficulty rather than reducing
it, since the hypothesis also implies the Collatz conjecture.
...
$L_c$ at one fixed $c$ carries none of this and is a strictly weaker assumption. Remarks elsewhere
...
4 & Total occupation $\Occ_c(m)=O(\log m)$ & Full EOC (Conjecture~\ref{conj:exist}); implies
    Level~3, and implied by its corridor-uniform form; strictness at a fixed corridor is
...
corridor --- then Corollary~\ref{cor:Uall} converts it into an $O(\log m)$ total-stopping-time
bound, and Proposition~\ref{prop:occstop} converts that into Level~4. So corridor-uniform
Level~3 \emph{implies} Level~4, and for that form strictness is false.

What survives: Level~4 implies Level~3 (immediately, since $L_c\le\Occ_c$); Level~1 is strictly
weaker than Collatz, which \emph{is} established, because $(\mathrm{DE})$ is compatible with
nontrivial cycles (Cor.~\ref{cor:cycles}); and for a \emph{fixed} corridor the converse
...
\texttt{SeparatedReturns} & Props.~\ref{prop:entry}, \ref{prop:episodes},
Rem.~\ref{rem:episodescope} & Restart relation; entry lemma; occupation ends at arrival
(Prop.~\ref{prop:occstop}); the oscillating word, its exit structure and unbounded realizable
episode counts. \\
